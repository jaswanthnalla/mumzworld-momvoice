"""Synthetic review generator for 3 Mumzworld products. No scraping."""
import json
import os
import random
...
OUT_DIR = Path(__file__).parent / "sample_products"
...
CHICCO_EN = [
    ...
    ("Delivery took 8 days and the box was damaged. Product itself is fine though.", 1, None),
    ("Courier left it outside in the heat. Mumzworld handled the replacement quickly.", 2, None),
    ("Packaging was excessive but the crib arrived intact.", 4, None),
]
...
CHICCO_AR = [
    ...
    ("التوصيل تأخر أسبوع كامل والكرتون كان مفتوح من جنب.", 2, None),
...
MEDELA_EN = [
    ...
    ("Box arrived dented but the pump was undamaged.", 3, None),
    ("Delivery was on time, packaging was good.", 5, None),
...
MEDELA_AR = [
    ...
    ("التوصيل سريع والتغليف ممتاز.", 5, None),
...
MACLAREN_EN = [
    ...
    ("Box arrived damaged but stroller was fine. Mumzworld helped with claim.", 3, None),
    ("Delivery took 6 days, longer than the 2 day estimate.", 2, None),
]
...
MACLAREN_AR = [
    ...
    ("التوصيل تأخر يومين عن الموعد المتوقع.", 3, None),
...
def build_dataset(name: str, product_name: str, en_pool, ar_pool, count_target=50):
    reviews = []
    rid = 1
    chosen = []
    for txt, rating, age in en_pool:
        chosen.append((txt, rating, "en", age))
    for txt, rating, age in ar_pool:
        chosen.append((txt, rating, "ar", age))

    if len(chosen) > count_target:
        chosen = chosen[:count_target]

    base_dates = ["2024-09-15", "2024-10-02", "2024-10-21", "2024-11-04", "2024-11-19",
                  "2024-12-08", "2024-12-22", "2025-01-10", "2025-01-28", "2025-02-14"]

    for i, (txt, rating, lang, age) in enumerate(chosen):
        reviews.append({
            "id": rid,
            "text": txt,
            "rating": rating,
    ...
    print(f"Wrote {path} with {len(reviews)} reviews")


def main():
    build_dataset("chicco_next2me_crib", "Chicco Next2Me Magic Crib", CHICCO_EN, CHICCO_AR)
    build_dataset("medela_swing_pump", "Medela Swing Maxi Double Breast Pump", MEDELA_EN, MEDELA_AR)
    build_dataset("maclaren_quest_stroller", "Maclaren Quest Stroller", MACLAREN_EN, MACLAREN_AR)
